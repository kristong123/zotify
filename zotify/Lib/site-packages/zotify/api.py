        ...
        class Metadata():
            ...
            def __init__(self, obj: Content, resp: dict):
                ...
                if isinstance(obj, Track) and isinstance(relative, Album):
                    ...
                    _album_rel = obj.parse_relatives([album], Album, make_parent=parent)
                    if _album_rel and _album_rel[0] is not None:
                        self.album              : Album             = _album_rel[0]
    ...
    def parse_relatives(self, resps: list[dict[str, str] | None], RelativeClasses: type[Content] | tuple[type[Content]],
        ...
        for i, resp in enumerate(resps):
            if not resp or not resp.get(TYPE) or not resp.get(URI):
                ...
                continue
            if resp[TYPE] not in type_selector: new_relatives.append(None); continue
        ...
        with Loader(f"Parsing {loader_text} information...", disabled=hide_loader):
            ...
            if recurs_objs:
                ...
                for recurse_type in contains if isinstance(contains, tuple) else (contains,):
                    recurse_uris = [item.uri for item in recurs_children if isinstance(item, recurse_type) and not item.is_local]
    ...
    def write_audio_tags(self, filepath: PurePath, parent_stack: ParentStack | None = None) -> None:
        ...
        def set_tag_safe(FILETAG, tag_value):
            ...
            try: file_tags[FILETAG] = tag_value
                ...
                Printer.hashtaged(PrintChannel.WARNING, f'FAILED TO SET TAG {FILETAG} FOR "{self.rel_path(filepath)}"\n' +
                                                        f'ERROR: {str(e)}')
        ...
        def custom_tag(tag: str, val: str):
            ...
            if self._ext in ("mp3", "aiff"):   custom_mp3_tag(tag, val)
        ...
        if self.album:
            if not self.album.image_url:
                try:
                    album_resp = type(self.album).fetch_metadata(self.album.uri)
                    self.album.parse_metadata(None, album_resp)
                except Exception:
                    pass
            ...
            if self.album.image_url:
                try:
                    ...
                    Printer.hashtaged(PrintChannel.WARNING, f'FAILED TO FETCH ALBUM ART FOR "{self.rel_path(filepath)}"\n' +
                                                            f'ERROR: {str(e)}')

        needs_fallback = (img is None
                          or not (self.album and self.album.year)
                          or not (self.album and self.album.artists)
                          or not self.track_number)
        if needs_fallback and not self.is_local:
            try:
                track_resp = type(self).fetch_metadata(self.uri)
                album_data = track_resp.get(ALBUM, {})
                if not (self.album and self.album.name):
                    set_tag_safe(ALBUM,       album_data.get(NAME))
                if not (self.album and self.album.year):
                    rd = album_data.get(RELEASE_DATE) or ''
                    set_tag_safe(YEAR,        rd[:4] if rd else None)
                if not (self.album and self.album.artists):
                    artist_names = [a.get(NAME, '') for a in album_data.get(ARTISTS, [])]
                    if artist_names:
                        set_tag_safe(ALBUMARTIST, Zotify.CONFIG.get_artist_delimiter().join(artist_names))
                if not self.track_number:
                    set_tag_safe(TRACKNUMBER, safe_typecast(track_resp, TRACK_NUMBER, int))
                if img is None:
                    images = album_data.get(IMAGES, [])
                    if images:
                        url = max(images, key=lambda i: i.get(WIDTH, 0) or 0).get(URL)
                        if url:
                            img = requests.get(url).content
                    elif album_data.get(COVER_GROUP):
                        covers = album_data[COVER_GROUP].get(IMAGE, [])
                        if covers:
                            best = max(covers, key=lambda c: safe_typecast(c, WIDTH, int) or 0)
                            fid = best.get(FILE_ID)
                            if fid:
                                img = requests.get(IMAGE_URL_PREFIX + Zotify.hex_id_from_file_id(fid)).content
            except Exception:
                pass

        if img and self._ext in ("mp3", "aiff"):
            from mutagen.id3 import APIC
            try:
                file_tags.mfile.tags.delall('APIC')
                file_tags.mfile.tags.add(APIC(encoding=3, mime='image/jpeg', type=3, desc='Cover', data=img))
            except Exception as e:
                Printer.hashtaged(PrintChannel.WARNING, f'FAILED TO SET TAG {ARTWORK} FOR "{self.rel_path(filepath)}"\n' +
                                                        f'ERROR: {str(e)}')
        elif img:
